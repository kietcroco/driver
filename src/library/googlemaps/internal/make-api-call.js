...
export var inject = (options) => {
  ...
  return (path, query, callback) => {
    ...
    var isPost = queryOptions.method && queryOptions.method.toLowerCase() === 'post';
  ...
  function formatRequestUrl(path, query, useClientId) {
    ...
    if (useClientId) {
      ...
      var signature = computeSignature(clientSecret, payload);
  ...
  function computeSignature(secret, payload) {
    ...
    //     .replace(/\//g, '_')
    //     .replace(/=+$/, '');
    var usablePrivateKey = new Buffer(websafe(secret), 'base64');
    var signature = CryptoJS.HmacSHA1(
      payload,
      CryptoJS.enc.Hex.parse(usablePrivateKey.toString('hex'))
